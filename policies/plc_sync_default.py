...
def sync_builtin_policies(tenant_sessions: list, logger):
    '''
        ...
        for plc in tqdm(tenant, desc='Syncing Default Policies', leave=False):
            for old_plc in original_tenant:
                if plc['name'] == old_plc['name']:
                    ...
                    compFlag = False
                    #check if there is cmp data to add
                    ...
                    #check if there is cmp data to remove/update
                    for el in complianceMetadata:
                        name = el['standardName']
                        if name not in [cmp['standardName'] for cmp in old_complianceMetadata]:
                            compFlag = True
                            break
                        req_id = el['requirementId']
                        if req_id not in [cmp['requirementId'] for cmp in old_complianceMetadata]:
                            compFlag = True
                            break
                        sec_id = el['sectionId']
                        if sec_id not in [cmp['sectionId'] for cmp in old_complianceMetadata]:
                            compFlag = True
                            break
...
if __name__ == '__main__':
    ...
    sync_builtin_policies(tenant_sessions)
